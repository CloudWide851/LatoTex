...
export function SubmissionEmailWorkbench(props: {
  ...
  const { busy, canUseRebuttal, emailConfigured, onOpenEmailSettings, onUseEmail, t } = props;
  ...
  const summary = useMemo(() => {
    ...
    if (items.length > 0) {
      return formatMessage(t("research.email.synced"), { count: items.length });
    }
    return emailConfigured ? t("research.email.empty") : t("research.email.configureTitle");
  }, [emailConfigured, items.length, message, syncBusy, t]);
  ...
  return (
    <div className="mt-3 grid min-w-0 grid-cols-[auto_minmax(0,1fr)] gap-2 rounded-md border border-[color:var(--editor-widget-border)] bg-[color:var(--editor-surface-bg)] p-2">
      <button
        ...
        disabled={syncBusy || !emailConfigured}
      ...
      <div className="min-w-0">
        <div className="flex min-w-0 items-center gap-1.5 text-xs text-[color:var(--editor-tab-muted)]">
          ...
          <span
            role={message && !message.ok ? "alert" : "status"}
            aria-live={message && !message.ok ? "assertive" : "polite"}
            className={[
              "min-w-0 truncate",
        ...
        <div className="mt-2 grid min-w-0 grid-cols-3 gap-2 max-[940px]:grid-cols-2 max-[700px]:grid-cols-1">
            ...
            return (
              ...
              >
                <div className="flex min-w-0 items-center gap-1.5">
                  <span className="min-w-0 flex-1 truncate text-xs font-semibold text-[color:var(--editor-tab-text)]">
                    {subject}
                  </span>
                  ...
                  <button
                    type="button"
                    className="panel-topbar-btn h-8 w-8 shrink-0 justify-center p-0 disabled:opacity-50"
                ...
                <div className="mt-1 truncate text-xs text-[color:var(--editor-tab-muted)]">
          ...
          {items.length === 0 ? (
            <div className="grid min-w-0 gap-1.5 rounded-md border border-dashed border-[color:var(--editor-widget-border)] px-3 py-2.5 text-xs text-[color:var(--editor-tab-muted)] sm:col-span-2 lg:col-span-3">
              <span className="font-semibold text-[color:var(--editor-tab-text)]">
                {emailConfigured ? t("research.email.empty") : t("research.email.configureTitle")}
              </span>
              <span className="max-w-2xl leading-5">
                {syncBusy
                  ? t("research.email.syncing")
                  : !emailConfigured
                    ? t("research.email.configureDescription")
                    : hasSynced
                      ? t("research.email.noMatchesDescription")
                      : t("research.email.readyDescription")}
              </span>
              <Button
                type="button"
                size="sm"
                variant="ghost"
                className="mt-0.5 w-fit px-2"
                onClick={onOpenEmailSettings}
              >
                {t("research.email.configureAction")}
              </Button>
            </div>
          ) : null}
        </div>
