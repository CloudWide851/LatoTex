...
export function StartupOverlay(props: {
  startupState: {
    ...
    error: string | null;
    currentStepKey?: AppStartupStep["key"] | null;
    analysisEnvStatus: {
      exists: boolean;
      venvPath: string;
      managedRoot: string;
  ...
  const visibleSteps = deriveStartupOverlaySteps(
    startupState.phase,
    startupState.steps,
    startupState.currentStepKey ?? null,
  );
  ...
  return (
    <div className="fixed inset-0 z-[460] flex items-center justify-center bg-slate-950/38 p-4 backdrop-blur-[2px] motion-overlay-enter">
      <section className="w-full max-w-lg rounded-[22px] border border-slate-200 bg-white/96 p-4 shadow-soft motion-card-pop motion-panel-glow sm:p-5">
        ...
        {visibleSteps.length > 0 ? (
          <div className="mt-4 space-y-2">
            {visibleSteps.map((step) => (
              <div
                key={step.key}
                className={`rounded-xl border px-3 py-2.5 ${step.status === "running"
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {step.detail ? (
                      <div className="mt-0.5 break-all text-xs leading-5 text-slate-500">{step.detail}</div>
                    ) : null}
                  </div>
        ...
        {failed && startupState.error ? (
          <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50/80 px-4 py-3 text-xs leading-5 text-amber-900">
            {startupState.error}
          </div>
        ) : null}
