...
fn builtin_zh_text(id: &str) -> Option<(&'static str, &'static str, Vec<&'static str>)> {
    match id {
        ...
        "latotex.toolchain.python" => Some((
            "Python 工具",
            "检测已配置的 Python，用于 Markdown 代码运行和数据工具。",
            vec!["工具链", "Python"],
        )),
        "latotex.toolchain.node" => Some((
            "Node.js 工具",
            "检测已配置的 Node.js，用于 Markdown 代码运行和项目工具。",
            vec!["工具链", "Node.js"],
        )),
        "latotex.toolchain.java" => Some((
            "Java JDK",
            "检测已配置的 Java JDK，用于 Markdown 代码运行和项目工具。",
            vec!["工具链", "编译器"],
        )),
...
pub(crate) fn built_in_catalog() -> Vec<PluginCatalogEntry> {
    ...
    vec![
        ...
        entry(toolchain_probe_manifest(
            "latotex.toolchain.python",
            "Python Tooling",
            "Detects a configured Windows x64 Python executable for Markdown code runs and project tools.",
            "python.windows-x64",
            "Python Windows x64",
            PluginToolchainProbe {
                id: "python".to_string(),
                ...
                platform: "windows-x64".to_string(),
                executables: vec!["python.exe".to_string()],
                version_arg: Some("--version".to_string()),
            },
            vec!["onCommand:toolchain.verify.python"],
            vec!["python", "interpreter"],
        )),
        entry(toolchain_probe_manifest(
            "latotex.toolchain.node",
            "Node.js Tooling",
            "Detects a configured Windows x64 Node.js executable for Markdown code runs and project tools.",
            "node.windows-x64",
            "Node.js Windows x64",
            PluginToolchainProbe {
                id: "node".to_string(),
                kind: "node".to_string(),
                platform: "windows-x64".to_string(),
                executables: vec!["node.exe".to_string()],
                version_arg: Some("--version".to_string()),
            },
            vec!["onCommand:toolchain.verify.node"],
            vec!["node", "javascript"],
        )),
        entry(toolchain_probe_manifest(
            "latotex.toolchain.java",
            "Java JDK",
            "Detects a configured Windows x64 Java JDK without running a global installer.",
            "java.windows-x64",
            "Java JDK Windows x64",
            PluginToolchainProbe {
                id: "java".to_string(),
                kind: "java".to_string(),
                platform: "windows-x64".to_string(),
                executables: vec!["javac.exe".to_string(), "java.exe".to_string()],
                version_arg: Some("-version".to_string()),
            },
            vec!["onCommand:toolchain.verify.java"],
            vec!["java", "jdk", "compiler"],
        )),
        entry(toolchain_probe_manifest(
