...
fn cache_package_dir_name(package_names: &[String], fallback: &str) -> String {
    package_names
        .iter()
        .find(|name| !name.trim().is_empty())
        .map(|name| name.trim().to_ascii_lowercase())
        .filter(|name| !name.is_empty())
        .unwrap_or_else(|| fallback.trim().to_ascii_lowercase())
}
...
pub fn busytex_install_missing_package(
    ...
    let package_cache_key = cache_package_dir_name(&package_names, &package_name);
    let policy = input
        .policy
        .as_deref()
    ...
    let cache_only = input.cache_only.unwrap_or(false);
    ...
    if has_required_style_file(&cached_overlay_files, &style_file) {
        return Ok(BusyTexInstallPackageResult {
            style_file,
            package_name: package_cache_key,
            installed: false,
            from_cache: true,
            source_url: None,
