...
fn paragraph_xml(body: &str, heading: u8, prefix: Option<&str>) -> String {
    let props = match heading {
        1 => r#"<w:pPr><w:pStyle w:val="Heading1"/></w:pPr>"#,
        2 => r#"<w:pPr><w:pStyle w:val="Heading2"/></w:pPr>"#,
        _ => "",
    };
    format!("<w:p>{props}{}</w:p>", inline_html_to_runs(body, heading, prefix))
}

fn table_xml(table: &str) -> String {
    ...
    for row in row_re.captures_iter(table) {
        ...
        for cell in cell_re.captures_iter(row_body) {
            ...
            xml.push_str(&paragraph_xml(cell_body, 0, None));
            xml.push_str("</w:tc>");
        }
        xml.push_str("</w:tr>");
    }
...
fn blocks_from_html(html: &str) -> Vec<String> {
    ...
    for block in block_re.captures_iter(html) {
        ...
        out.push(paragraph_xml(body, heading, prefix.as_deref()));
    }
    if out.is_empty() {
        let text = strip_tags(html);
        vec![paragraph_xml(&html_escape(&text), 0, None)]
    } else {
        out
    }
}
...
fn document_xml_from_html(html: &str) -> String {
    let blocks = blocks_from_html(html).into_iter().collect::<String>();
    format!(
        r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body>{blocks}<w:sectPr><w:pgSz w:w="12240" w:h="15840"/><w:pgMar w:top="1440" w:right="1440" w:bottom="1440" w:left="1440"/></w:sectPr></w:body></w:document>"#
    )
}
...
fn replace_document_xml(original: &[u8], document_xml: &str) -> Result<Vec<u8>, String> {
...
pub async fn docx_write(state: State<'_, AppState>, input: DocxWriteInput) -> Result<Ack, String> {
    ...
    let db_path = state.db_path.clone();
    spawn_blocking(move || {
        let document_xml = document_xml_from_html(&input.html);
        let bytes = match storage::read_project_file_binary(&db_path, &input.project_id, &input.relative_path) {
            Ok(existing) => replace_document_xml(&existing.bytes, &document_xml)
